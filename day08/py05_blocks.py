...
class Block:
    ...
    def move(self): # 공 움직임
        ...
        # 볼이 움직이는 y축 값을 계속 계산하려면 y는 dir값을 radians으로 변환 후 사인으로 처리
        self.rect.centery -= math.sin(math.radians(self.dir)) * self.speed
        
    def draw_E(self): # 공을 circle이 아니라 ellipse로 생성
        pygame.draw.ellipse(Surface, self.col, self.rect) #rect의 위치에 col 색으로     
...
def main():
    is_game_start = False # 게임시작
    score = 0
    
    BLOCK =[]
    BALL = Block((200,200,0), Rect(375,650,20,20),10)  # 공 생성, 공 스피드 10으로 시작
    PADDLE = Block((200,200,0), Rect(375, 700,100,30)) # 공을 맞출 패들 생성
    ...
    # 초기에 생성될 블럭들(무지개색으로 아홉개씩, 54개의 블록을 만든다)
